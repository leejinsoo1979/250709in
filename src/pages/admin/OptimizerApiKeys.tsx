...
import styles from './OptimizerApiKeys.module.css';

interface OptimizerApiKey {
  id: string;
  name: string;
  key: string;
  userId: string;
...
const OptimizerApiKeys = () => {
  const { user } = useAuth();
  const [apiKeys, setApiKeys] = useState<OptimizerApiKey[]>([]);
  ...
  const loadApiKeys = async () => {
    ...
    try {
      ...
      const keysQuery = query(
        collection(db, 'optimizerApiKeys'),
        where('userId', '==', user.uid)
      );
      const snapshot = await getDocs(keysQuery);
      const keysList: OptimizerApiKey[] = [];
  ...
  const handleCreateKey = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, 'optimizerApiKeys'), {
  ...
  const handleDeleteKey = async (id: string, name: string) => {
    ...
    try {
      await deleteDoc(doc(db, 'optimizerApiKeys', id));
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <div>
          <h1 className={styles.title}>컷팅 옵티마이저 API 키 관리</h1>
          <p className={styles.subtitle}>컷팅 옵티마이저 서비스에서 사용할 API 키를 생성하고 관리합니다.</p>
        </div>
        <button
...
export default OptimizerApiKeys;
