...
describe('convertPlacedPanelToMprBoringData', () => {
  ...
  it('exports furniture side panel dimensions as height by depth like the reference MPR', () => {
    const panel = {
      id: 'side-reference-size',
      name: '우측',
      width: 380,
      height: 1000,
      x: 0,
      ...
      color: 'MW',
      grain: 'VERTICAL',
      sideNotches: [{ y: 70, z: 40, fromBottom: 930 }],
      bracketBoringPositions: [120],
    ...
    expect(converted.height).toBe(380);
    expect(mpr).toContain('l="1000.0000"');
    expect(mpr).toContain('w="380.0000"');
    expect(mpr).toContain('X=0.0000');
    expect(mpr).toContain('Y=360.0000');
    expect(mpr).toContain('X=1000.0000');
    expect(mpr).toContain('Y=40.0000');
    expect(mpr).toContain('X=930.0000');
    expect(mpr).toContain('Y=0.0000');
    expect(mpr).toContain('XA="-1.0000"');
    expect(mpr).toContain('YA="361.5000"');
    expect(mpr).toContain('XE="1001.0000"');
    expect(mpr).toContain('YE="361.5000"');
    expect(converted.borings.filter(boring => boring.note === 'door-fixing-screw')).toEqual([
      expect.objectContaining({ x: 880, y: 20 }),
      expect.objectContaining({ x: 880, y: 52 }),
    ]);
  });
  ...
  it('exports split side panel grooves/notches with the same left-right rule as optimizer preview', () => {
    ...
    expect(upperLeftMpr).toContain('X=0.0000');
    expect(upperLeftMpr).toContain('Y=360.0000');
    expect(upperLeftMpr).toContain('XA="-1.0000"');
    expect(upperLeftMpr).toContain('YA="361.5000"');
    ...
    expect(lowerRightMpr).toContain('Y=360.0000');
    expect(lowerRightMpr).toContain('XA="-1.0000"');
    expect(lowerRightMpr).toContain('YA="361.5000"');
    expect(lowerRightMpr).toContain('X=780.0000');
