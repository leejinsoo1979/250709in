...
function isFurnitureRightSidePanel(panel: PlacedPanel): boolean {
  const name = panel.name || '';
  return !name.includes('서랍')
    && !name.includes('도어')
    && !name.includes('Door')
    && (name.includes('우측판') || name.includes('우측'));
}

function resolveMprTopBoringX(panel: PlacedPanel, x: number): number {
  ...
  if (!panel.isDoor && !isDrawerFrontPanel && panel.boringPositions && panel.boringPositions.length > 0) {
    ...
    yPositions.forEach((yPos) => {
      ...
      xPositionsForY.forEach((xPos: number) => {
        const mprX = isFurnitureSidePanelForMpr ? yPos : resolveMprTopBoringX(panel, xPos);
        const mprY = isFurnitureSidePanelForMpr ? resolveMprSidePanelDepthY(panel, xPos) : yPos;
        borings.push({
    ...
    panel.bracketBoringPositions.forEach((yPos) => {
      bracketXPositions.forEach((xPos) => {
        borings.push({
          ...
          type: 'hinge-screw',
          face: 'top',
          x: isFurnitureSidePanelForMpr ? yPos : xPos,
          y: isFurnitureSidePanelForMpr ? xPos : yPos,
          diameter: 3,
          depth: 3,
          note: 'door-fixing-screw',
        });
