    ...
    result.panels.forEach((panel, panelIndex) => {
      ...
      if (showBorings && isDoorPanel && hasDoorBoringData) {
        ...
        doorPoints.cupPoints.forEach((point) => {
          ...
          if (isHighlighted) {
            drawBoringCoordinateTag(ctx, {
              label: `${formatPanelMm(point.x)}/${formatPanelMm(point.y)}`,
              x: bx,
              y: by,
              panelX: x,
              panelY: y,
      ...
      if (showGrooves && isFurnitureSidePanel) {
        ...
        const backPanelDepthOffset = panel.backPanelGroove?.offset ?? 16; // mm (측판 뒤쪽 끝에서 백패널 홈 시작까지 거리)
        const grooveWidth = panel.backPanelGroove?.width ?? 10; // mm (홈 폭)
