...
export interface PlacedModule {
  ...
  depthFrontReferenceMm?: number; // 앞고정 전환/유지 시 기준이 되는 앞라인 깊이(mm)
  ...
  topFrameThickness?: number; // 자유배치 상단몰딩 두께 (mm, undefined = 공간 설정값 사용)
  ...
  hasBottomEndPanel?: boolean;   // 상부장 하부 EP 표시 여부
  ...
  userResizedHeight?: boolean; // 사용자가 직접 높이를 변경함 (이동 시 자동 높이 복원 차단)
  ...
  // N섹션 가구 — 각 섹션의 깊이 (mm). 인덱스: 0=1단, 1=2단, 2=3단
  sectionDepths?: number[];
  // N섹션 가구 — 각 섹션의 깊이 줄임 방향 ('front'=앞, 'back'=뒤)
  ...
  // 상판설치 (하부장 전용)
  stoneTopMaterial?: 'stone';                    // 상판 재질 (인조대리석, 기본: stone)
