    ...
    <>
      ...
      {showFurniture && (
        <>
          ...
          {/* 다보 선반 렌더링 (기본하부장/도어올림/상판내림 반통·한통·2단·3단) */}
          {(() => {
            ...
            if (isTopDownHalf) {
              ...
              // 기본하부장 반통/한통: 상판 없음 → 바닥판 위 ~ 캐비넷 상단
              referenceHeightMm = cabinetHeightMm - basicThicknessMm;
            }

            // 3등분 → 선반 2개 위치 (바닥판 위 기준)
            const shelfInterval = referenceHeightMm / 3;
            const shelfPositions = [shelfInterval, shelfInterval * 2]; // mm from bottom panel top

            // 선반 치수
            const shelfThicknessMm = 18; // 다보 선반 두께
            const shelfFrontGapMm = 30; // 앞에서 30mm 떨어짐
            const shelfDepthMm = depthMm - backPanelMm - shelfFrontGapMm;
            const shelfWidth = baseFurniture.innerWidth; // 좌우 측판 사이
            const shelfDepth = mmToUnits(shelfDepthMm);
            const shelfThickness = mmToUnits(shelfThicknessMm);

            // Z 위치: 백패널 앞면 ~ 앞에서 30mm 지점의 중앙
