...
const TouchDrawerAnimated: React.FC<TouchDrawerAnimatedProps> = ({
  ...
  const renderTouchDrawerBody = (d: typeof drawers[number], i: number) => (
          <React.Fragment key={`touch-drawer-${i}`}>
            {/* 바닥판 (반턱) — N(인너서랍): 전면 속마이다(20mm) 자리만큼 앞 감소.
                바닥판 아랫면 = 이격 입력값(정의 보존) — 기성 뒷판과의 맞물림은 GLB를 9.3mm 내려서 해결 */}
            {(() => {
...
const LowerCabinet: React.FC<FurnitureTypeProps> = ({
  ...
  return (
    <>
      ...
      {showFurniture && !moduleData.id.includes('dishwasher') && (
        <AdjustableFootsRenderer
