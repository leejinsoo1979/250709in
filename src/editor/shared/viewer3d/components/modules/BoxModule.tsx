...
const BoxModule: React.FC<BoxModuleProps> = ({
  ...
  // === 하부장 타입들 (싱글 및 듀얼) ===
  if (moduleData.id.includes('lower-cabinet-') || moduleData.id.includes('dual-lower-cabinet-') || moduleData.id.includes('lower-half-cabinet') || moduleData.id.includes('dual-lower-half-cabinet') || moduleData.id.includes('lower-drawer-') || moduleData.id.includes('lower-door-lift-') || moduleData.id.includes('lower-top-down-')) {
    return (
      <>
        {/* 모든 타입에서 간접조명 렌더링 */}
        {/* IndirectLight는 마지막에 한 번만 렌더링 */}
    ...
    <>
      ...
      {showFurniture && (
        <BaseFurnitureShell
          ...
          panelGrainDirections={panelGrainDirections}
          moduleData={moduleData}
          lowerSectionTopOffsetMm={(moduleData?.id?.includes('entryway-h') || moduleData?.id?.includes('entryway-i')) ? 85 : (lowerSectionTopOffset || 0)}
        >
          ...
          {(
            <SectionsRenderer
              ...
              shelfFrontInsetMm={(moduleData?.id?.includes('entryway-h') || moduleData?.id?.includes('entryway-i')) ? 30 : 0}
            />
          )}
        </BaseFurnitureShell>
      )}
