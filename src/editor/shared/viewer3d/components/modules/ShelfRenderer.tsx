...
export const ShelfRenderer: React.FC<ShelfRendererProps> = ({
  ...
  if (isTopFinishPanel && shelfCount === 1) {
    // 최상단 마감 패널 모드 (18mm 아래로 조정)
    const topPosition = innerHeight / 2 - basicThickness / 2 - mmToThreeUnits(18);
    ...
    return (
      <group position={[0, yOffset, 0]}>
        {shelfPositions.map((positionMm, i) => {
          ...
          // positionMm이 0이면 바닥판(중간 구분 패널 바로 위에 배치)
          const relativeYPosition = positionMm === 0 
            ? (-innerHeight / 2) + basicThickness / 2 + basicThickness  // 바닥판: 중간 구분 패널(basicThickness) 위
