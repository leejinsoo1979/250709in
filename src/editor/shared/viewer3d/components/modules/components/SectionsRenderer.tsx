    ...
    return allSections.map((section: SectionConfig & { calculatedHeight: number }, index: number) => {
      ...
      return (
        <group key={`section-${index}`}>
          ...
          {/* 첫 번째 섹션의 하단 프레임 두께 표시 - DualType4는 제외 (DualType4.tsx에서 직접 렌더링) */}
          {showDimensions && showDimensionsText && !(viewMode === '2D' && (view2DDirection === 'left' || view2DDirection === 'right' || view2DDirection === 'top')) && index === 0 && !(furnitureId?.includes('dual-4drawer-hanging')) && (
            <group>
