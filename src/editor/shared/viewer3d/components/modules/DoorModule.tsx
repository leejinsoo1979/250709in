...
const DoorModule: React.FC<DoorModuleProps> = ({
  ...
  // 도어 위치 계산: 간단한 로직
  // 싱글과 듀얼 모두 slotCenterX 사용 (엔드패널 보정 포함)
  // slotCenterX는 가구 중심에서의 오프셋
  let doorGroupX = slotCenterX !== undefined ? slotCenterX : 0;
  ...
  if (isDualFurniture) {
    ...
    // 듀얼 캐비넷 도어: 도어 간 3mm 갭 유지
    // 각 슬롯에서 3mm 빼면 중앙에 3mm 갭 생성
    leftDoorWidth = slot1Width - 3;  // 슬롯 크기 - 3mm (양쪽 1.5mm씩)
    rightDoorWidth = slot2Width - 3; // 슬롯 크기 - 3mm (양쪽 1.5mm씩)
    ...
    // 갭 검증: 슬롯에서 2mm 뺐으므로 양쪽 1mm씩 갭
    const leftGapCheck = (slot1Width - leftDoorWidth) / 2; // 1mm가 나와야 함
    const rightGapCheck = (slot2Width - rightDoorWidth) / 2; // 1mm가 나와야 함
    
    // 듀얼 캐비넷 내부 갭 (두 도어 사이)
    const internalGap = 1 + 1; // 양쪽 1mm씩 = 2mm 갭
    ...
    // 싱글 캐비넷 도어: 도어 간 3mm 갭 유지
    // 듀얼 캐비넷과 동일한 규칙 적용: 슬롯 - 3mm (양쪽 1.5mm씩)
