...
const PlacedFurnitureContainer: React.FC<PlacedFurnitureContainerProps> = ({
  ...
  const { activePopup, view2DDirection: contextView2DDirection, selectedSlotIndex, selectedFurnitureId, isLiveDimensionMode, isTapeMeasureMode } = useUIStore();
  ...
  return (
    <group name="FurnitureContainer">
      {filteredModules.map((placedModule) => {
        ...
        const isInspecting3D = viewMode === '3D' && (isLiveDimensionMode || isTapeMeasureMode);
