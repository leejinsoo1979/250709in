...
const ColumnAsset: React.FC<ColumnAssetProps> = ({
  ...
  const handlePointerDown = (event: ThreeEvent<PointerEvent>) => {
    ...
    let localDragStart = event.point;
    ...
    const handleGlobalPointerMove = (e: PointerEvent) => {
      ...
      const columnDepthM = depth * 0.01; // mm를 적절한 단위로 변환 
  ...
  return (
    <group position={position}>
      ...
      {(isHovered || isDragging) && (
        <mesh position={[0, (height * 0.01) / 2, 0]}>
          <boxGeometry args={[width * 0.01 + 0.02, height * 0.01 + 0.02, depth * 0.01 + 0.02]} />
          <meshBasicMaterial 
            color={isDragging ? "#ff6b6b" : "#4CAF50"} 
            transparent 
            opacity={0.2} 
            wireframe 
          />
