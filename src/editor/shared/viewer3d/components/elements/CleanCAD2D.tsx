  ...
  const renderFrontView = () => (
    <group position={[0, 0, zOffset]} renderOrder={9999}>
      ...
      <group>
            {/* 치수선 */}
            <Line
              points={[[leftOffset, slotTotalDimensionY, 0.002], [leftOffset + mmToThreeUnits(frameSize.left), slotTotalDimensionY, 0.002]]}
              color={dimensionColor}
              lineWidth={0.5}
            />
            ...
            {/* 좌측 화살표 */}
            <Line
              points={createArrowHead([leftOffset, slotTotalDimensionY, 0.002], [leftOffset + 0.02, slotTotalDimensionY, 0.002])}
              color={dimensionColor}
              lineWidth={0.5}
            />
            ...
            {/* 우측 화살표 */}
            <Line
              points={createArrowHead([leftOffset + mmToThreeUnits(frameSize.left), slotTotalDimensionY, 0.002], [leftOffset + mmToThreeUnits(frameSize.left) - 0.02, slotTotalDimensionY, 0.002])}
              color={dimensionColor}
              lineWidth={0.5}
            />
            ...
            <Text
                  renderOrder={1000}
                  depthTest={false}
              position={[leftOffset + mmToThreeUnits(frameSize.left) / 2, slotTotalDimensionY + mmToThreeUnits(30), 0.01]}
              fontSize={baseFontSize}
              color={textColor}
              anchorX="center"
              anchorY="middle"
            ...
            />
            <NativeLine name="dimension_line"
              points={[[leftOffset + mmToThreeUnits(frameSize.left), spaceHeight, 0.001], [leftOffset + mmToThreeUnits(frameSize.left), slotTotalDimensionY + mmToThreeUnits(EXTENSION_LENGTH), 0.001]]}
              color={dimensionColor}
              lineWidth={1.5}
              renderOrder={1000000}
              depthTest={false}
      ...
      <group>
            {/* 치수선 */}
            <Line
              points={[[mmToThreeUnits(spaceInfo.width) + leftOffset - mmToThreeUnits(frameSize.right), slotTotalDimensionY, 0.002], [mmToThreeUnits(spaceInfo.width) + leftOffset, slotTotalDimensionY, 0.002]]}
              color={dimensionColor}
              lineWidth={0.5}
            />
            ...
            {/* 좌측 화살표 */}
            <Line
              points={createArrowHead([mmToThreeUnits(spaceInfo.width) + leftOffset - mmToThreeUnits(frameSize.right), slotTotalDimensionY, 0.002], [mmToThreeUnits(spaceInfo.width) + leftOffset - mmToThreeUnits(frameSize.right) + 0.02, slotTotalDimensionY, 0.002])}
              color={dimensionColor}
              lineWidth={0.5}
            />
            ...
            {/* 우측 화살표 */}
            <Line
              points={createArrowHead([mmToThreeUnits(spaceInfo.width) + leftOffset, slotTotalDimensionY, 0.002], [mmToThreeUnits(spaceInfo.width) + leftOffset - 0.02, slotTotalDimensionY, 0.002])}
              color={dimensionColor}
              lineWidth={0.5}
            />
            ...
            <Text
                  renderOrder={1000}
                  depthTest={false}
              position={[mmToThreeUnits(spaceInfo.width) + leftOffset - mmToThreeUnits(frameSize.right) / 2, slotTotalDimensionY + mmToThreeUnits(30), 0.01]}
              fontSize={baseFontSize}
              color={textColor}
              anchorX="center"
              anchorY="middle"
            ...
            {/* 연장선 - topDimensionY(3600선)까지 연장 */}
            <NativeLine name="dimension_line"
              points={[[mmToThreeUnits(spaceInfo.width) + leftOffset - mmToThreeUnits(frameSize.right), spaceHeight, 0.001], [mmToThreeUnits(spaceInfo.width) + leftOffset - mmToThreeUnits(frameSize.right), slotTotalDimensionY + mmToThreeUnits(EXTENSION_LENGTH), 0.001]]}
              color={dimensionColor}
              lineWidth={1.5}
              renderOrder={1000000}
              depthTest={false}
        ...
        {(() => {
          ...
          return (
            <>
              ...
              {/* ── 도어 높이 치수선 (도어 설치 시에만 표시) ── */}
              {rightmostMod?.hasDoor && (() => {
                // 도어 높이 치수선 X 위치: 커튼박스가 있으면 4단, 없으면 3단 위치
                const rightDoorDimX = hasCB_R
                  ? rightWallX + mmToThreeUnits(800)
                  : rightWallX + mmToThreeUnits(600);

                // 가구 카테고리 판별
                const rDoorModData = getModuleById(
                  rightmostMod.moduleId,
                  { width: spaceInfo.width, height: spaceInfo.height, depth: spaceInfo.depth },
                  spaceInfo
                );
                const rDoorCategory = rDoorModData?.category
                  ?? (rightmostMod.moduleId.includes('-upper-') ? 'upper'
                    : rightmostMod.moduleId.includes('-lower-') ? 'lower' : 'full');

                let doorHeightMm = 0;
                let doorBottomAbsMm = 0; // 바닥에서 도어 하단까지 (mm)
                let doorTopAbsMm = 0;    // 바닥에서 도어 상단까지 (mm)

                const doorTopGapVal = rightmostMod.doorTopGap ?? 5;
                const doorBottomGapVal = rightmostMod.doorBottomGap ?? 25;

                if (rDoorCategory === 'upper') {
                  // 상부장: 도어가 캐비넷 바깥으로 확장
                  const cabinetH = rDoorModData?.dimensions.height ?? 600;
                  const topFrameVal = rightmostMod.topFrameThickness ?? (spaceInfo.frameSize?.top ?? 30);
                  const topExtension = topFrameVal - doorTopGapVal;
                  doorHeightMm = cabinetH + topExtension + doorBottomGapVal;
                  doorTopAbsMm = rEffectiveH - doorTopGapVal;
                  doorBottomAbsMm = doorTopAbsMm - doorHeightMm;
                } else if (rDoorCategory === 'lower') {
                  // 하부장
                  const cabinetH = rDoorModData?.dimensions.height ?? 1000;
                  const isDoorLift = rDoorModData?.id?.includes('lower-door-lift-');
                  const isTopDown = rDoorModData?.id?.includes('lower-top-down-');
                  const cabinetBottomAbs = rFloorFinishForHeight + rBottomFrameH;
                ...
                return (
                  <>
                    {/* 세로 메인 라인 */}
                    <NativeLine name="dimension_line"
                      points={[[rightDoorDimX, doorBottomY, 0.002], [rightDoorDimX, doorTopY, 0.002]]}
                      color="#E91E63" lineWidth={1} renderOrder={100000} depthTest={false}
                    />
                    {/* 화살표 */}
                    <NativeLine name="dimension_line"
                      points={createArrowHead([rightDoorDimX, doorBottomY, 0.002], [rightDoorDimX, doorBottomY + 0.05, 0.002])}
                      color="#E91E63" lineWidth={1} renderOrder={100000} depthTest={false}
                    />
                    <NativeLine name="dimension_line"
                      points={createArrowHead([rightDoorDimX, doorTopY, 0.002], [rightDoorDimX, doorTopY - 0.05, 0.002])}
                      color="#E91E63" lineWidth={1} renderOrder={100000} depthTest={false}
                    />
                    {/* 도어 높이 텍스트 */}
                    <Text renderOrder={1000} depthTest={false}
                      position={[rightDoorDimX + mmToThreeUnits(10), doorMidY, 0.01]}
                      fontSize={largeFontSize} color="#E91E63"
                      anchorX="left" anchorY="middle"
                      outlineWidth={textOutlineWidth} outlineColor={textOutlineColor}
                    >
                    ...
                    {/* 연장선: 도어 상단/하단 */}
                    <NativeLine name="dimension_line"
                      points={[[rightWallX, doorTopY, 0.001], [rightDoorDimX + mmToThreeUnits(20), doorTopY, 0.001]]}
                      color="#E91E63" lineWidth={1} renderOrder={100000} depthTest={false}
                    />
                    <NativeLine name="dimension_line"
                      points={[[rightWallX, doorBottomY, 0.001], [rightDoorDimX + mmToThreeUnits(20), doorBottomY, 0.001]]}
                      color="#E91E63" lineWidth={1} renderOrder={100000} depthTest={false}
                    />
                  </>
                );
