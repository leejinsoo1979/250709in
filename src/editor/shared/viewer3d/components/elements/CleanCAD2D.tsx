...
// console.log('❌ showDimensions is false, returning null');
  ...
  const renderFrontView = () => (
    <group position={[0, 0, zOffset]} renderOrder={9999}>
        ...
        {spaceInfo.droppedCeiling?.enabled ? (
          <>
            ...
            {spaceInfo.droppedCeiling.position === 'left' ? (
              <>
                {/* 좌측 단내림/커튼박스 - 좌측 외부 치수선에 높이차이 표시 */}
                {(() => {
                  // 자유배치: dropHeight 구간은 height ~ height+dropHeight (위로 확장)
                  // 슬롯배치: dropHeight 구간은 height-dropHeight ~ height (아래로 축소)
                  ...
                  return (
                    <>
                      ...
                      <Text
                        renderOrder={1000}
                        depthTest={false}
                        position={[leftDimensionX + leftOffset - mmToThreeUnits(60), dimMid, 0.01]}
                        fontSize={largeFontSize}
                        color={textColor}
                        anchorX="center"
                        anchorY="middle"
      ...
      {/* 슬롯 배치 모드에서 가구 없을 때 슬롯 폭 치수선 미리 표시 */}
      {showDimensions && !isFreePlacement && !hasPlacedModules && currentViewDirection !== 'top' && indexing.slotWidths && indexing.slotWidths.length > 0 && (() => {
        const dimY = slotDimensionY;
        const extLen = mmToThreeUnits(EXTENSION_LENGTH);

        return (
          <group key="empty-slot-widths">
            {indexing.slotWidths.map((slotWidth, i) => {
              const slotLeft = threeUnitBoundaries[i];
              const slotRight = threeUnitBoundaries[i + 1];
              if (slotLeft === undefined || slotRight === undefined) return null;
              const centerX = (slotLeft + slotRight) / 2;
              const displayWidth = Math.round(slotWidth * 10) / 10;

              return (
                <group key={`empty-slot-${i}`}>
                  {/* 슬롯 폭 치수선 */}
                  <NativeLine name="dimension_line"
                    points={[[slotLeft, dimY, 0.002], [slotRight, dimY, 0.002]]}
                    color={dimensionColor}
                    lineWidth={1}
                    renderOrder={100000}
                    depthTest={false}
                  ...
                  {/* 좌측 화살표 */}
                  <NativeLine name="dimension_line"
                    points={createArrowHead([slotLeft, dimY, 0.002], [slotLeft + 0.02, dimY, 0.002])}
                    color={dimensionColor}
                    lineWidth={1}
                    renderOrder={100000}
                    depthTest={false}
                  ...
                  {/* 우측 화살표 */}
                  <NativeLine name="dimension_line"
                    points={createArrowHead([slotRight, dimY, 0.002], [slotRight - 0.02, dimY, 0.002])}
                    color={dimensionColor}
                    lineWidth={1}
                    renderOrder={100000}
                    depthTest={false}
                  ...
                  <Text
                    renderOrder={1000}
                    depthTest={false}
                    position={[centerX, dimY + mmToThreeUnits(20), 0.01]}
                    fontSize={baseFontSize}
                    color={textColor}
                    anchorX="center"
                    anchorY="middle"
                    ...
                    outlineColor={textOutlineColor}
                  >
                    {displayWidth % 1 === 0 ? displayWidth.toFixed(0) : displayWidth.toFixed(1)}
                  </Text>
                  {/* 좌측 보조선 */}
                  <NativeLine name="dimension_line"
                    points={[[slotLeft, spaceHeight, 0.001], [slotLeft, dimY + extLen, 0.001]]}
                    color={dimensionColor}
                    lineWidth={1}
                    renderOrder={100000}
                    depthTest={false}
                  />
                  {/* 우측 보조선 (마지막 슬롯만) */}
                  {i === indexing.slotWidths!.length - 1 && (
                    <NativeLine name="dimension_line"
                      points={[[slotRight, spaceHeight, 0.001], [slotRight, dimY + extLen, 0.001]]}
                      color={dimensionColor}
                      lineWidth={1}
                      renderOrder={100000}
                      depthTest={false}
