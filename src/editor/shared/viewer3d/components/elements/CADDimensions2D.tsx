...
const CADDimensions2D: React.FC<CADDimensions2DProps> = ({ viewDirection, showDimensions: showDimensionsProp }) => {
  ...
  return (
    <group>
      ...
      {/* 측면도용 가구 깊이 치수 - 좌측/우측 뷰에서만 표시 */}
      {(currentViewDirection === 'left' || currentViewDirection === 'right') && React.useMemo(() => placedModules.map((module, index) => {
        ...
        const floatHeight = spaceInfo?.baseConfig?.placementType === 'float'
          ? mmToThreeUnits(spaceInfo.baseConfig.floatHeight || 0)
          : 0;
        const furnitureY = module.position.y;

        // 치수선 위치 (가구 왼쪽, 높이 중간)
        const dimX = -furnitureDepth / 2 - mmToThreeUnits(100);
        const frontZ = furnitureDepth / 2;
        const backZ = -furnitureDepth / 2;

        return (
          <group key={`side-furniture-dim-${index}`}>
            {/* 가구 깊이 치수선 (수평) */}
            <Line
              points={[
                [dimX, furnitureY, backZ],
                [dimX, furnitureY, frontZ]
              ]}
              color={dimensionColors.furniture}
              lineWidth={2}
              renderOrder={1000}
            ...
            <Line
              points={createArrow(
                new THREE.Vector3(dimX, furnitureY, frontZ),
                new THREE.Vector3(dimX, furnitureY, frontZ - 0.03),
                0.015
              )}
              color={dimensionColors.furniture}
              lineWidth={2}
            ...
            <Line
              points={createArrow(
                new THREE.Vector3(dimX, furnitureY, backZ),
                new THREE.Vector3(dimX, furnitureY, backZ + 0.03),
                0.015
              )}
              color={dimensionColors.furniture}
              lineWidth={2}
            ...
            {/* 치수 텍스트 */}
            <Html
              position={[dimX - mmToThreeUnits(50), furnitureY, 0]}
              center
              transform={false}
              occlude={false}
              zIndexRange={[1000, 1001]}
      ...
      }), [placedModules, spaceInfo, currentViewDirection])}
