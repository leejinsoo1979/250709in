...
const CustomizablePropertiesPanel: React.FC = () => {
  ...
  const renderElementEditor = (
    ...
    // 상부섹션(최상단)에는 서랍 불가 (위에 공간이 없어 인출 불가)
    // 그 외 섹션(하부/중간/단독)에는 모든 타입 허용
    const isUpperSection = config.sections.length > 1 && sIdx === config.sections.length - 1;
  ...
  const renderSubSplitElementEditor = (
    ...
    // 상부에는 서랍 불가 (인출 공간 부족), 하부에는 모든 타입 허용
        ...
        <div key={`${sIdx}-${areaSide}`}>
          ...
          <div className={styles.section}>
            ...
            <div className={styles.row}>
              ...
              <div className={styles.toggleGroup}>
                <button
                  className={`${styles.toggleButton} ${(section.areaFinish?.[areaSide]?.showBackPanel ?? section.showBackPanel) !== false ? styles.active : ''}`}
                  onClick={() => {
                    const sections = [...config.sections];
                    const sec = { ...sections[sIdx] };
                    sec.areaFinish = { ...sec.areaFinish, [areaSide]: { ...sec.areaFinish?.[areaSide], showBackPanel: true } };
                    sections[sIdx] = sec;
                    applyConfig({ ...config, sections });
                  }}
                >있음</button>
                <button
                  ...
                  onClick={() => {
                    const sections = [...config.sections];
                    const sec = { ...sections[sIdx] };
                    sec.areaFinish = { ...sec.areaFinish, [areaSide]: { ...sec.areaFinish?.[areaSide], showBackPanel: false } };
                    sections[sIdx] = sec;
                    applyConfig({ ...config, sections });
                  }}
                >없음</button>
            ...
            <div className={styles.row}>
              ...
              <input
                ...
                value={section.areaFinish?.[areaSide]?.bottomPanelRaise ?? section.bottomPanelRaise ?? 0}
                onChange={(e) => {
                  const v = e.target.value.replace(/[^0-9]/g, '');
                  const sections = [...config.sections];
                  const sec = { ...sections[sIdx] };
                  sec.areaFinish = { ...sec.areaFinish, [areaSide]: { ...sec.areaFinish?.[areaSide], bottomPanelRaise: v === '' ? 0 : parseInt(v) } };
                  sections[sIdx] = sec;
                  applyConfig({ ...config, sections });
                }}
                onBlur={(e) => {
                  ...
                  const sections = [...config.sections];
                  const sec = { ...sections[sIdx] };
                  sec.areaFinish = { ...sec.areaFinish, [areaSide]: { ...sec.areaFinish?.[areaSide], bottomPanelRaise: clamped } };
                  sections[sIdx] = sec;
                  applyConfig({ ...config, sections });
                }}
                onKeyDown={(e) => {
                  ...
                  if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
                    e.preventDefault();
                    const cur = section.areaFinish?.[areaSide]?.bottomPanelRaise ?? section.bottomPanelRaise ?? 0;
                    const delta = e.key === 'ArrowUp' ? 1 : -1;
                    const maxRaise = section.height || 500;
                    const next = Math.max(0, Math.min(maxRaise, cur + delta));
                    const sections = [...config.sections];
                    const sec = { ...sections[sIdx] };
                    sec.areaFinish = { ...sec.areaFinish, [areaSide]: { ...sec.areaFinish?.[areaSide], bottomPanelRaise: next } };
                    sections[sIdx] = sec;
                    applyConfig({ ...config, sections });
                  }
                }}
                    ...
                    {placedModule.hasLeftEndPanel && (() => {
                      ...
                      return (
                        <div style={{ marginTop: '4px' }}>
                          ...
                          <span style={{ fontSize: '10px', color: 'var(--theme-text-tertiary)', marginTop: '2px', display: 'block' }}>+ 앞 확장 / - 뒤 축소</span>
                        </div>
                      );
                    })()}
                    {/* 우측 EP 옵셋 */}
