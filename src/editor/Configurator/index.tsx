                      ...
                      {freeBaseEnabled ? (
                        <div style={{ display: 'flex', flex: 1, gap: '4px' }}>
                          ...
                          <div className={styles.frameItemInput} style={{ flex: 1 }}>
                            ...
                            <input
                              type="text" inputMode="numeric"
                              value={mod.baseFrameOffset ?? (spaceInfo.baseConfig?.height ?? 65)} placeholder={String(spaceInfo.baseConfig?.height ?? 65)}
                              onFocus={() => setHighlightedFrame(`base-${mod.id}`)}
                              onKeyDown={(e) => {
                                if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
                                  e.preventDefault();
                                  const cur = mod.baseFrameOffset ?? (spaceInfo.baseConfig?.height ?? 65);
                              ...
                              onBlur={(e) => { setHighlightedFrame(null); updatePlacedModule(mod.id, { baseFrameOffset: Math.max(-200, Math.min(200, parseInt(e.target.value) || (spaceInfo.baseConfig?.height ?? 65))) }); }}
                              className={styles.frameNumberInput}
                            />
                          </div>
                        </div>
              ...
              {enabled ? (
                <div style={{ display: 'flex', flex: 1, gap: '4px' }}>
                  ...
                  <div className={styles.frameItemInput} style={{ flex: 1 }}>
                    ...
                    <input
                      type="text" inputMode="numeric"
                      value={offset} placeholder={String(globalBase)}
                      onFocus={() => setHighlightedFrame(hlKey)}
                      onKeyDown={(e) => {
                        if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
                          e.preventDefault();
                      ...
                      onBlur={(e) => {
                        setHighlightedFrame(null);
                        onOffsetChange(Math.max(-200, Math.min(200, parseInt(e.target.value) || globalBase)));
                      }}
                      className={styles.frameNumberInput}
                    />
                  </div>
                ...
                {enabled ? (
                  <div style={{ display: 'flex', flex: 1, gap: '4px' }}>
                    ...
                    <div className={styles.frameItemInput} style={{ flex: 1 }}>
                      ...
                      <input
                        type="text" inputMode="numeric"
                        value={mod.baseFrameOffset ?? globalBase} placeholder={String(globalBase)}
                        onFocus={() => setHighlightedFrame(`base-${mod.id}`)}
                        onKeyDown={(e) => {
                          if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
                            e.preventDefault();
                            const cur = mod.baseFrameOffset ?? globalBase;
                            updatePlacedModule(mod.id, { baseFrameOffset: Math.max(-200, Math.min(200, cur + (e.key === 'ArrowUp' ? 1 : -1))) });
                          }
                        }}
                        onChange={(e) => { const v = e.target.value; if (v === '' || v === '-' || /^-?\d+$/.test(v)) updatePlacedModule(mod.id, { baseFrameOffset: v === '' || v === '-' ? 0 : parseInt(v, 10) }); }}
                        onBlur={(e) => { setHighlightedFrame(null); updatePlacedModule(mod.id, { baseFrameOffset: Math.max(-200, Math.min(200, parseInt(e.target.value) || globalBase)) }); }}
                        className={styles.frameNumberInput}
                      />
                    </div>
                  </div>
                  ...
                  <div className={styles.subSetting}>
                    ...
                    {spaceInfo.baseConfig?.type !== 'stand' && baseGroups.map((group, gIdx) => {
                      ...
                      return <React.Fragment key={`merged-base-${gIdx}`}>{renderMergedFrameRow(
                        ...
                        firstMod?.baseFrameOffset ?? globalBase,
                        () => {
            ...
            {hasDoorsInstalled && !isMobile && (() => {
              ...
              return (
                <div
                  ...
                  style={{ position: 'absolute', top: 10, left: '50%', transform: 'translateX(-50%)', zIndex: 100, width: 90, height: 28, borderRadius: 14, background: dark ? 'rgba(255,255,255,0.15)' : 'rgba(0,0,0,0.1)', cursor: 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center', padding: 2, boxSizing: 'border-box', position: 'absolute', userSelect: 'none' }}
                >
                  {/* 슬라이딩 활성 배경 */}
