...
const clampFrameGapValue = (value: number): number => Math.max(-2000, Math.min(2000, value));
const clampFloatHeightValue = (value: number): number => Math.max(0, Math.min(500, value));
...
const FrameOffsetRow: React.FC<{
  ...
  const commitGap = (nextGap: number) => {
    const clampedGap = clampFrameGapValue(nextGap);
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '3px 0' }}>
      <span className={styles.frameItemLabel} style={{ minWidth: '34px', textAlign: 'left', margin: 0 }}>{num > 0 ? toAlpha(num) : ''}{label}</span>
...
const Configurator: React.FC = () => {
  ...
  const getShelfSplitTopClearanceUpdates = useCallback((mod: any, nextState: Record<string, any>, effectiveHeight = spaceInfo.height) => {
    ...
    const nextTopGap = nextState.topFrameGap ?? mod.topFrameGap ?? computeShelfSplitTopDistance(mod, effectiveHeight) ?? ((spaceInfo.frameSize as any)?.topGap ?? 0);
    ...
    (spaceInfo.frameSize as any)?.topGap,
  ...
  const handleIndividualDoorGapChange = (moduleId: string, field: DoorGapField, val: string, syncModuleIds?: string[]) => {
    const targetModule = useFurnitureStore.getState().placedModules.find(m => m.id === moduleId);
    const isPantrySplit = !!targetModule?.moduleId?.includes('pantry-cabinet-split');
    if (isPantrySplit && (field === 'upperDoorBottomGap' || field === 'lowerDoorTopGap')) {
      return;
    }
    ...
    return (
      <div className={`${styles.spaceControls} ${isFreeMode ? styles.spaceControlsRelaxed : ''}`}>
        ...
        {isFreeMode && (() => {
                  ...
                  {enabled ? (
                    ...
                    <div style={numCellStyle}>
                      ...
                      <input type="text" inputMode="numeric" value={floatHeight || ''}
                        onChange={(e) => { const v = e.target.value; if (v === '' || /^\d+$/.test(v)) onFloat(v === '' ? 0 : clampFloatHeightValue(parseInt(v, 10))); }}
                        style={{ ...numInputStyle, color: 'var(--theme-text-primary)' }} />
                    </div>
                  )}
                </div>
                      ...
                      : baseFrameSlots.map((slot, idx) => {
                        ...
                        return renderBaseGuideRow(
                          ...
                          (v, nextSize) => updateGuideSlotFrame(slot.id, {
                            ...
                            baseFrameGap: clampFrameGapValue(v)
                          }),
          ...
          const defaultIfZeroFree = (value: number | undefined, fallback: number) => (
            value === undefined ? fallback : value
          );
          return (
            <>
                  ...
                  (() => {
                    ...
                    const furnitureActualH = firstTop.freeHeight
                      ...
                      ?? (() => {
	                    ...
	                    const currentGap = firstTop.userResizedHeight === true
	                      ? (computeShelfSplitTopDistance(firstTop) ?? computedGap)
	                      : (firstTop.topFrameGap ?? computeShelfSplitTopDistance(firstTop) ?? computedGap);
                    return (
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '3px 0' }}>
                        <span className={styles.frameItemLabel} style={{ minWidth: '34px', textAlign: 'left', margin: 0 }}>전체</span>
	                        <button
		                          onClick={() => topFreeMods.forEach(m => {
		                            const nextTopSize = getFreeTopOffGap(m);
		                            updatePlacedModule(m.id, getShelfSplitTopClearanceUpdates(m, {
                          ...
                          <div className={styles.frameItemInput} style={{ flex: 1 }}>
                            ...
                            <input
                              ...
                              onKeyDown={(e) => {
                                if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
                                  e.preventDefault();
                                  const nextGap = clampFrameGapValue(currentGap + (e.key === 'ArrowUp' ? 1 : -1));
                                  topFreeMods.forEach(m => updatePlacedModule(m.id, getShelfSplitTopClearanceUpdates(m, { topFrameGap: nextGap })));
                                }
                              }}
                              onChange={(e) => { const v = e.target.value; if (v === '' || v === '-' || /^-?\d+$/.test(v)) { const nextGap = v === '' || v === '-' ? 0 : parseInt(v, 10); topFreeMods.forEach(m => updatePlacedModule(m.id, getShelfSplitTopClearanceUpdates(m, { topFrameGap: nextGap }))); } }}
                              onBlur={(e) => { const nextGap = clampFrameGapValue(parseInt(e.target.value) || 0); topFreeMods.forEach(m => updatePlacedModule(m.id, getShelfSplitTopClearanceUpdates(m, { topFrameGap: nextGap }))); }}
                              className={styles.frameNumberInput}
                            />
                          </div>
                        </div>
                ...
                sorted.map((mod) => {
                  ...
                  if (cat === 'upper') {
                    ...
                    return <FrameOffsetRow key={`top-${mod.id}`}
                      ...
                      gap={defaultIfZeroFree(mod.topFrameGap, globalTopGapLocal)}
                      onToggle={() => {
                  ...
                  return <FrameOffsetRow key={`top-${mod.id}`}
                    ...
                    enabled={mod.hasTopFrame !== false} sizeMM={actualTopFrameSize} offset={mod.topFrameOffset ?? ((spaceInfo.frameSize as any)?.topOffset ?? 0)}
	                    gap={mod.hasTopFrame === false
	                      ? (mod.userResizedHeight === true ? actualTopFrameSize : defaultIfZeroFree(mod.topFrameGap, actualTopFrameSize))
	                      : defaultIfZeroFree(mod.topFrameGap, globalTopGapLocal)}
	                      onToggle={() => {
	                        const newVal = !(mod.hasTopFrame !== false);
	                      updatePlacedModule(mod.id, newVal ? {
	                        ...getTopFrameSizeUpdates(mod, actualTopFrameSize, effectiveSpaceHeight),
	                        hasTopFrame: true,
	                        topFrameGap: 0,
		                        doorTopGap: getTopDoorGapForFrameState(spaceInfo, true, mod)
	                      } : {
	                        hasTopFrame: false,
	                        topFrameGap: actualTopFrameSize,
            ...
            <div className={styles.configSection}>
              ...
              <div className={styles.subSetting}>
                          ...
                          <div className={styles.frameItemInput} style={{ flex: 1 }}>
                            ...
                            <input
                              ...
                              onKeyDown={(e) => {
                                if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
                                  e.preventDefault();
                                  const nv = clampFloatHeightValue(currentFloat + (e.key === 'ArrowUp' ? 1 : -1));
                                  baseFreeMods.forEach(m => updatePlacedModule(m.id, getShelfSplitTopClearanceUpdates(m, { individualFloatHeight: nv })));
                                }
                              }}
                              onChange={(e) => { const v = e.target.value; if (v === '' || /^\d+$/.test(v)) { const nv = v === '' ? 0 : clampFloatHeightValue(parseInt(v, 10)); baseFreeMods.forEach(m => updatePlacedModule(m.id, getShelfSplitTopClearanceUpdates(m, { individualFloatHeight: nv }))); } }}
                              onBlur={(e) => { const nv = clampFloatHeightValue(parseInt(e.target.value) || 0); baseFreeMods.forEach(m => updatePlacedModule(m.id, getShelfSplitTopClearanceUpdates(m, { individualFloatHeight: nv }))); }}
                              className={styles.frameNumberInput}
                            />
                          </div>
                        </div>
                ...
                sorted.map((mod) => {
                  ...
                  const baseOffsetDefault = cat === 'lower'
                    ? (spaceInfo.baseboardLowerOffset ?? (spaceInfo.baseConfig as any)?.offset ?? 0)
                      ...
                      <button
                        onClick={() => {
                          updatePlacedModule(mod.id, {
                            ...
                            ...(freeBaseEnabled ? { individualFloatHeight: clampFloatHeightValue(defaultIfZeroFree((mod as any).baseFrameGap, globalBaseGapLocal)) } : {}),
                          });
                        }}
                      ...
                      {freeBaseEnabled ? (
                        <div style={{ display: 'flex', flex: 1, gap: '4px' }}>
                          ...
                          <div className={styles.frameItemInput} style={{ flex: 1 }}>
                            ...
                            <input
                              type="text" inputMode="numeric"
                              value={defaultIfZeroFree(mod.baseFrameOffset, baseOffsetDefault)} placeholder={String(baseOffsetDefault)}
                              onFocus={() => setHighlightedFrame(`base-${mod.id}`)}
                              onKeyDown={(e) => {
                                if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
                                  e.preventDefault();
                                  const cur = defaultIfZeroFree(mod.baseFrameOffset, baseOffsetDefault);
                              ...
                              onBlur={(e) => { setHighlightedFrame(null); updatePlacedModule(mod.id, { baseFrameOffset: Math.max(-200, Math.min(200, parseInt(e.target.value) || baseOffsetDefault)) }); }}
                              className={styles.frameNumberInput}
                            />
                          </div>
                        </div>
                            ...
                            <input
                              ...
                              onKeyDown={(e) => {
                                if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
                                  ...
                                  updatePlacedModule(mod.id, getShelfSplitTopClearanceUpdates(mod, { individualFloatHeight: clampFloatHeightValue(cur + (e.key === 'ArrowUp' ? 1 : -1)) }));
                                }
                              }}
                              onChange={(e) => { const v = e.target.value; if (v === '' || /^\d+$/.test(v)) updatePlacedModule(mod.id, getShelfSplitTopClearanceUpdates(mod, { individualFloatHeight: v === '' ? 0 : clampFloatHeightValue(parseInt(v, 10)) })); }}
                              onBlur={(e) => { updatePlacedModule(mod.id, getShelfSplitTopClearanceUpdates(mod, { individualFloatHeight: clampFloatHeightValue(parseInt(e.target.value) || 0) })); }}
                              className={styles.frameNumberInput}
                            />
                          </div>
                        </div>
        ...
        {!isFreeMode && (() => {
          ...
          const renderSlotFrameRow = (
            ...
            <FrameOffsetRow
              ...
              splitGapFromSize={splitGapFromSize}
              toAlpha={toAlpha}
              styles={styles}
                ...
                <button
                  onClick={() => {
                    updatePlacedModule(mod.id, getShelfSplitTopClearanceUpdates(mod, {
                      ...
                      ...(enabled ? { individualFloatHeight: clampFloatHeightValue(baseGap) } : {}),
                    }));
                  }}
                  className={`${styles.miniToggle} ${enabled ? styles.miniToggleActive : ''}`}
                />
                    ...
                    <div className={styles.frameItemInput} style={{ flex: 1 }}>
                      ...
                      <input
                        ...
                        onBlur={(e) => {
                          ...
                          const clampedGap = clampFrameGapValue(Number.isFinite(nextGap) ? nextGap : 0);
                          updatePlacedModule(mod.id, {
              ...
              {enabled ? (
                <div style={{ display: 'flex', flex: 1, gap: '4px' }}>
                  ...
                  <div className={styles.frameItemInput} style={{ flex: 1 }}>
                    ...
                    <input
                      type="text" inputMode="numeric"
                      value={offset} placeholder={String(globalBaseOffset)}
                      onFocus={() => setHighlightedFrame(hlKey)}
                      onKeyDown={(e) => {
                        if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
                          e.preventDefault();
                      ...
                      return <React.Fragment key={`merged-top-${gIdx}`}>{renderMergedFrameRow(
                        ...
                        firstMod?.topFrameOffset ?? topOffsetDefault,
	                          () => {
	                            ...
	                            updatePlacedModule(id, target ? getShelfSplitTopClearanceUpdates(target, newVal ? {
	                              ...getTopFrameSizeUpdates(target, computeShelfSplitTopDistance(target) ?? defaultIfZeroSlot(target.topFrameGap, globalTopGap)),
	                              hasTopFrame: true,
	                              topFrameGap: 0,
		                              doorTopGap: getTopDoorGapForFrameState(spaceInfo, true, target),
	                            } : {
	                              hasTopFrame: false,
	                              topFrameGap: computeShelfSplitTopDistance(target) ?? defaultIfZeroSlot(target.topFrameGap, globalTopGap),
		                              doorTopGap: getTopDoorGapForFrameState(spaceInfo, false, target),
                    ...
                    {allTopOn && topSortedMods.length > 0 ? (
	                        ...
	                        return renderSlotFrameRow(
	                          ...
	                          () => {
	                            ...
	                            topSortedMods.forEach(m => updatePlacedModule(m.id, getShelfSplitTopClearanceUpdates(m, {
	                              ...
	                              ...(newVal
	                                ? getTopFrameSizeUpdates(m, computeShelfSplitTopDistance(m) ?? defaultIfZeroSlot(m.topFrameGap, globalTopGap))
	                                : {}),
	                            ...
	                            <button
	                              onClick={() => {
		                                topSortedMods.forEach(m => {
		                                  const floorFinishForMod = spaceInfo.hasFloorFinish && spaceInfo.floorFinish?.height ? spaceInfo.floorFinish.height : 0;
		                                  const bottomForMod = m.hasBase === false
		                                    ? (m.individualFloatHeight ?? 0)
		                                    : (m.baseFrameHeight ?? globalBase);
		                                  const furnitureHForMod = m.freeHeight
		                                    ?? m.customHeight
		                                    ?? (() => {
		                                      const md = getModuleById(m.moduleId, { width: spaceInfo.width, height: spaceInfo.height, depth: spaceInfo.depth }, spaceInfo);
		                                      return md?.dimensions.height ?? 0;
		                                    })();
		                                  const nextTopSize = m.userResizedHeight === true
		                                    ? (computeShelfSplitTopDistance(m) ?? Math.max(0, spaceInfo.height - furnitureHForMod - bottomForMod - floorFinishForMod))
		                                    : (computeShelfSplitTopDistance(m) ?? defaultIfZeroSlot(m.topFrameGap, globalTopGap));
		                                  updatePlacedModule(m.id, getShelfSplitTopClearanceUpdates(m, {
		                                    ...getTopFrameSizeUpdates(m, nextTopSize),
		                                    hasTopFrame: true,
		                                    topFrameGap: 0,
                              ...
                              <div className={styles.frameItemInput} style={{ flex: 1 }}>
                                ...
                                <input
                                  ...
                                  onChange={(e) => {
                                    const v = e.target.value;
                                    if (v === '' || v === '-' || /^-?\d+$/.test(v)) {
                                      const nextGap = v === '' || v === '-' ? 0 : parseInt(v, 10);
                                      topSortedMods.forEach(m => updatePlacedModule(m.id, getShelfSplitTopClearanceUpdates(m, { topFrameGap: nextGap })));
                                    }
                                  }}
                                  onBlur={(e) => {
                                    const nextGap = clampFrameGapValue(parseInt(e.target.value) || 0);
                                    topSortedMods.forEach(m => updatePlacedModule(m.id, getShelfSplitTopClearanceUpdates(m, { topFrameGap: nextGap })));
                                  }}
                                  className={styles.frameNumberInput}
                                />
                      ...
                      sorted.map((mod) => {
                        ...
                        const topOffsetDefault = (spaceInfo.frameSize as any)?.topOffset ?? ((cat === 'upper' && spaceInfo.surroundType === 'surround') ? 23 : 0);
	                        const floorFinishH = spaceInfo.hasFloorFinish && spaceInfo.floorFinish?.height ? spaceInfo.floorFinish.height : 0;
	                        const bottomPortion = mod.hasBase === false
	                          ? (mod.individualFloatHeight ?? 0)
	                          : (mod.baseFrameHeight ?? globalBase);
	                        const furnitureActualH = mod.freeHeight
	                          ?? mod.customHeight
	                          ?? (() => {
	                            const md = getModuleById(mod.moduleId, { width: spaceInfo.width, height: spaceInfo.height, depth: spaceInfo.depth }, spaceInfo);
	                            return md?.dimensions.height ?? 0;
	                          })();
	                        const computedTopOffGap = Math.max(0, spaceInfo.height - furnitureActualH - bottomPortion - floorFinishH);
	                        const displayedTopOffGap = mod.userResizedHeight === true
	                          ? (computeShelfSplitTopDistance(mod) ?? computedTopOffGap)
	                          : (computeShelfSplitTopDistance(mod) ?? defaultIfZeroSlot(mod.topFrameGap, globalTopGap));
                          ...
                          () => {
	                            const newVal = !(mod.hasTopFrame !== false);
	                            updatePlacedModule(mod.id, getShelfSplitTopClearanceUpdates(mod, newVal ? {
	                              ...getTopFrameSizeUpdates(mod, displayedTopOffGap),
	                              hasTopFrame: true,
	                              topFrameGap: 0,
		                              doorTopGap: getTopDoorGapForFrameState(spaceInfo, true, mod)
	                            } : {
	                              hasTopFrame: false,
	                              topFrameGap: displayedTopOffGap,
		                              doorTopGap: getTopDoorGapForFrameState(spaceInfo, false, mod)
	                            }));
	                          },
                          ...
                          `top-${mod.id}`,
	                          mod.hasTopFrame === false ? displayedTopOffGap : defaultIfZeroSlot(mod.topFrameGap, globalTopGap),
