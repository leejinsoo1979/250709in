    ...
    return (
      <div className={`${styles.spaceControls} ${isFreeMode ? styles.spaceControlsRelaxed : ''}`}>
        ...
        {!isFreeMode && (() => {
          ...
          const globalBaseHeight = spaceInfo.baseConfig?.height ?? 65;
          ...
          return (
            <div className={styles.configSection}>
              <div className={styles.sectionHeader} onClick={() => setIsFrameSectionCollapsed(prev => !prev)} style={{ cursor: 'pointer', userSelect: 'none' }}>
                <span className={styles.sectionDot}></span>
                <h3 className={styles.sectionTitle}>상,하부프레임</h3>
                <HelpBtn title="상,하부프레임" text="선택된 가구의 상부/하부 프레임 높이를 개별 설정합니다. 상부프레임: 가구 위쪽 마감 패널. 하부프레임: 가구 아래쪽 받침대 높이." />
                <IoIosArrowDropup style={{ marginLeft: 'auto', fontSize: '14px', color: 'var(--theme-text-secondary)', transition: 'transform 0.2s', transform: isFrameSectionCollapsed ? 'rotate(180deg)' : 'rotate(0deg)' }} />
              </div>
              {!isFrameSectionCollapsed && (
                <div className={styles.subSetting}>
                  {/* 상부프레임 */}
                  {renderSlotFrameRow('A(상)',
                    selMod.hasTopFrame !== false,
                    selMod.topFrameThickness ?? globalTopFrame,
                    selMod.topFrameOffset ?? 0,
                    () => updatePlacedModule(selMod.id, { hasTopFrame: !(selMod.hasTopFrame !== false) }),
                    (v) => updatePlacedModule(selMod.id, { topFrameThickness: v }),
                    (v) => updatePlacedModule(selMod.id, { topFrameOffset: v }),
                    `top-${selMod.id}`,
                  )}
                  {/* 하부프레임 — stand 타입이면 숨김 */}
                  {spaceInfo.baseConfig?.type !== 'stand' && renderSlotFrameRow('A(하)',
                    selMod.hasBase !== false,
                    selMod.baseFrameHeight ?? globalBaseHeight,
                    selMod.baseFrameOffset ?? 0,
                    () => updatePlacedModule(selMod.id, { hasBase: !(selMod.hasBase !== false) }),
                    (v) => updatePlacedModule(selMod.id, { baseFrameHeight: v }),
                    (v) => updatePlacedModule(selMod.id, { baseFrameOffset: v }),
                    `base-${selMod.id}`,
                  )}
                </div>
              )}
